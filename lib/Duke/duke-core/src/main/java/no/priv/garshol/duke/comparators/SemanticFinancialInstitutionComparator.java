 ...
 * On top of it, a pruning step is performed in order to avoid false positives.
 * The pruning process is based on extraction of REGEX such as "bank". 
 * It is implemented inside the similarity function.
 */

public class SemanticFinancialInstitutionComparator implements Comparator {
  ...
  public static double similarity(String s1, String s2) {
   ...
   //after computing the JW distance we apply the pruning process

   //define a set of keywords

    String[] keywords = {"bank", "bancshares", "corporation", "holding", "trust", "bancorp", "capital","federal savings bank", "incorporated", "investment", "services"};
    
    //create arrays for BoW representation
    
    Object[] BOW1 = new Object[keywords.length]; 
	   
    Object[] BOW2 = new Object[keywords.length]; 


    //iterate over keywords in the string and populate the BOW
    
    for (int i = 0; i < keywords.length; i++){
		   
    String key = keywords[i]; //current keyword
		   
    //check on the first string
    if (Arrays.asList(s1.split(" ")).contains(key)){
       BOW1[i] = 1;
      }
		   
    //check on the second string

    if (Arrays.asList(s2.split(" ")).contains(key)){
	BOW2[i] = 1;
		   }		   
	}
  
	if (Arrays.deepEquals(BOW1,BOW2)){
